...
<div class="flex flex-col scroll-padrao px-2 pb-12 pt-[30px] py-3 gap-5 overflow-x-hidden overflow-y-auto h-full  w-full " bind:this={containerChat}>
    {#each chatAtual.mensagens as mensagem,i (mensagem.id) }
    {@const x = 900}
            {#if mensagem.bot}
                <div class="flex flex-row gap-2 items-center"
                in:fly|global={{x:x*-1,easing:expoOut,duration:1200}}
                out:fly|global={{x:x*-1,easing:expoOut,duration:600}}
                >
                    <TitleBlock classPosTitle="top-[-50%] left-[50%]" title={mensagem.modelo} className="p-3 rounded-full shadow-md self-end flex items-center justify-center w-12 h-12 aspect-square bg-white">
            ...
            {:else}
                <div class="ml-auto flex max-w-[80%] shadow-md flex-col gap-2 rounded-l-xl rounded-tr-xl bg-black p-4 text-[16px] text-neutral-100
                 ...
                 in:fly|global={{x:x,easing:expoOut,duration:1200}}
                 out:fly|global={{x:x,easing:expoOut,duration:600}}>
                 {#if mensagem.arquivos}
                    {#each mensagem.arquivos}
                         <div class="absolute whitespace-nowrap markdown flex gap-1 items-center  top-[-30px] right-0">
                            ...
                            {mensagem.arquivos.length} file(s) sended
                         </div>
                    {/each}
                 {/if}  
                 {mensagem.conteudo}
